...
const MIGRATION_SQL = `
-- Create all koperasi tables if not exist
...
DECLARE v_count INT; v_kode VARCHAR(50);
BEGIN
...
export async function runKoperasiMigration() {
  try {
    ...
    if (check?.[0]?.exists) return { message: 'Tables already exist' }

    // Split by semicolons and execute each
    const stmts = MIGRATION_SQL.split(';').map(s => s.trim()).filter(s => s.length > 0 && !s.startsWith('--'))
    let ok = 0, fail = 0
    for (const stmt of stmts) {
      try {
        ...
        console.error('Migration stmt failed:', e?.message?.slice(0, 150))
        fail++
      }
    }
    return { message: `Migration done: ${ok} ok, ${fail} failed` }
  } catch (err: any) {
