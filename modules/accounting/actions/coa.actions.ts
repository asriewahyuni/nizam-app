...
// ─────────────────────────────────────────────────────────────
// resetCoA — Hapus seluruh akun CoA organisasi dan seed ulang ke PSAK standar.
// Hanya bisa dilakukan jika tidak ada satu pun akun yang sudah bertransaksi.
// ─────────────────────────────────────────────────────────────
export async function resetCoA(orgId: string) {
  const supabase = await createClient()
  const trimmedOrgId = String(orgId || '').trim()
  ...
  // Guard: pastikan pengguna boleh kelola CoA langsung
  const { canManageDirect } = await checkCanManageCoA(trimmedOrgId)
  if (!canManageDirect) {
    return { error: 'Hanya admin organisasi utama yang dapat mereset CoA.' }
  }

  // Guard: cek apakah ada akun yang sudah memiliki transaksi journal
  const { count: journalCount } = await (supabase as any)
    .from('journal_lines')
    .select('*', { count: 'exact', head: true })
    .in(
  ...
  if ((journalCount ?? 0) > 0) {
    return {
      error: 'CoA tidak dapat direset karena sudah ada transaksi jurnal yang menggunakan akun ini. Hubungi tim dukungan jika perlu bantuan.',
    }
  }

  // Hapus semua akun organisasi ini
  ...
  if (deleteError) {
    ...
    return { error: msg ? `Gagal menghapus CoA: ${msg}` : 'Gagal menghapus CoA.' }
  }

  // Seed ulang dengan PSAK standar
  const { error: rpcError } = await (supabase as any).rpc('seed_default_coa', { p_org_id: trimmedOrgId })
  if (rpcError) {
    return { error: 'CoA berhasil dihapus, namun gagal seed ulang PSAK. Klik "Aktifkan CoA PSAK" untuk melanjutkan.' }
  }
