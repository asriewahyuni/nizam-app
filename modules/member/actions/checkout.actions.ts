...
export async function createMemberCheckout(
  ...
  const guestEmail = normalizeEmail(cleanText(formData.get('email'), 200))
  const guestPhone = normalizePhone(cleanText(formData.get('phone'), 40))
  ...
  try {
    ...
    if (existing.rows[0]) {
      ...
      if (checkoutUser.created && checkoutUser.internalUserId) {
        ...
        const claim = await client.query<{ id: string }>(
          ...
          [
            offering.org_id,
            checkoutUser.internalUserId,
            checkoutUser.userId,
            orderId,
            claimTokenHash,
          ],
        )
        const claimUrl = `${appBaseUrl()}/member/${orgSlug}/klaim-akun?token=${encodeURIComponent(claimToken)}`
        await client.query(
          ...
          [
            ...
            checkoutUser.userId,
            checkoutUser.email,
            `Halo ${checkoutUser.name}, aktifkan akun belajar Anda melalui tautan ini: ${claimUrl}. Tautan berlaku 48 jam dan hanya dapat dipakai sekali.`,
            JSON.stringify({
