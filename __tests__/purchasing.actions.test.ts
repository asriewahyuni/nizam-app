...
describe('Purchasing Branch Context', () => {
  ...
  it('falls back to merged shipping amount when purchases.insurance_amount is missing during published header sync', async () => {
    const productsQuery = createNoopMutationBuilder()
        ...
        return {
          eq: vi.fn(() => ({
            eq: vi.fn().mockResolvedValue(
              Object.prototype.hasOwnProperty.call(payload, 'insurance_amount')
                ? {
                    error: {
                      code: '42703',
                      message: 'column "insurance_amount" of relation "purchases" does not exist',
                    },
                  }
                : { error: null }
            ),
    ...
    const rpcMock = vi.fn().mockResolvedValue({
      data: { success: true, purchase_id: 'po-legacy-ins-1' },
      error: null,
    })
    ...
    expect(result).toEqual({ success: true, purchaseId: 'po-legacy-ins-1' })
    ...
    expect(updatePayloads[1]).not.toHaveProperty('insurance_amount')
  })
