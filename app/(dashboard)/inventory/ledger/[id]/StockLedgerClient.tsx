  ...
  return (
    <div className="max-w-6xl mx-auto space-y-10 pb-20">
      ...
      <div className="flex flex-col gap-6">
        <Link 
          href="/inventory" 
          className="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-wide hover:text-slate-900 transition-colors w-fit group"
        >
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <h1 className="text-3xl font-semibold text-slate-900 tracking-tight flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-blue-600 text-white flex items-center justify-center shadow-lg shadow-indigo-100">
                <History size={24} />
              </div>
          ...
          <div className="flex gap-4">
             <div className="bg-white px-6 py-4 rounded-xl border border-slate-100 shadow-sm flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center">
                   <ArrowDownRight size={20} />
                </div>
                <div>
                   <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide leading-none mb-1">Total Masuk</p>
             ...
             <div className="bg-white px-6 py-4 rounded-xl border border-slate-100 shadow-sm flex items-center gap-4">
                ...
                <div>
                   <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide leading-none mb-1">Total Keluar</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         <div className="bg-slate-900 text-white rounded-xl p-8 relative overflow-hidden shadow-md shadow-indigo-500/10 md:col-span-1">
            <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/20 rounded-full blur-3xl -mr-10 -mt-10" />
            <div className="relative z-10">
               <p className="text-[10px] font-semibold uppercase text-blue-400 tracking-wide mb-2">Stok Akhir</p>
               <h4 className="text-5xl font-semibold tracking-tighter">{currentBalance}</h4>
               <p className="text-xs font-bold text-slate-400 mt-2 uppercase tracking-tight">{product?.unit || 'Unit'}</p>
            </div>
         </div>
         <div className="bg-white rounded-xl p-8 border border-slate-100 shadow-sm md:col-span-3 flex items-center justify-around">
            <div className="text-center space-y-2">
               <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Transaksi Terakhir</p>
               <div className="flex items-center gap-2 text-slate-900 font-semibold justify-center">
            ...
            <div className="text-center space-y-2">
               <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Aset Persediaan</p>
               <div className="flex items-center gap-2 text-slate-900 font-semibold justify-center">
            ...
            <div className="w-px h-12 bg-slate-100" />
            <div className="text-center space-y-2">
               <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Total Pergerakan</p>
               <div className="flex items-center gap-2 text-slate-900 font-semibold justify-center">
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden min-h-[400px]">
         <div className="p-8 border-b border-slate-50 bg-slate-50/30 flex items-center justify-between">
            ...
            <div className="px-5 py-2.5 bg-white border border-slate-200 rounded-xl text-[10px] font-semibold uppercase text-slate-400">
                    ...
                    <tr>
                      <td colSpan={7} className="px-10 py-20 text-center">
                         <div className="flex flex-col items-center gap-3 opacity-20">
                            <Package size={48} />
                            <p className="text-sm font-semibold uppercase tracking-wide">Belum ada pergerakan stok.</p>
                         </div>
                      </td>
                    </tr>
                  ) : (
                    ledgerData.map((m: any, idx: number) => (
                      <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-6 py-6">
                           <div className={`px-3 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wide w-fit mb-1.5 border
                           ...
                           <p className="text-xs font-semibold text-slate-900 font-mono tracking-tighter">REF-{m.reference_id?.substring(0,8)}</p>
                        </td>
                        ...
                        <td className="px-6 py-6 text-right">
                           {m.quantity < 0 && (
                             <div className="flex items-center justify-end gap-1.5 text-rose-600 font-semibold">
                                {m.quantity}
                                <ArrowUpRight size={14} />
                             </div>
                           )}
                        </td>
                        <td className="px-6 py-6 text-right font-semibold text-blue-600 bg-blue-50/10 text-lg tracking-tighter">
                           {m.running_balance}
                        </td>
      ...
      {/* Footer Info */}
      <div className="bg-blue-50/50 rounded-xl p-6 border border-blue-100 flex items-start gap-4">
         <Info className="text-blue-500 shrink-0 mt-0.5" size={20} />
         <div className="space-y-1">
            <h4 className="text-xs font-semibold text-blue-900 uppercase">Audit Compliance</h4>
