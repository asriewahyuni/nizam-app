...
import { CheckCircle2, Factory, Settings, Zap } from 'lucide-react'
import { SimpleSettingsForm, CompleteOnboardingButton, type SettingsField } from '@/components/shared/ModuleOnboardingActions'

const MODULE_KEY = 'Manufacturing'
const SETTINGS_FIELDS: SettingsField[] = [
  { name: 'default_production_unit', label: 'Satuan Produksi Default', type: 'text', placeholder: 'cth: Unit, Kg, Liter', defaultValue: 'Unit' },
  { name: 'bom_categories', label: 'Kategori BoM', type: 'text', placeholder: 'cth: Produksi, Perakitan, Campuran', defaultValue: 'Produksi' },
]
...
export default async function ManufacturingOnboardingPage() {
  noStore()
  const orgData = await getActiveOrg()
  if (!orgData) redirect('/onboarding')
  const instance = await getModuleInstanceStatus(orgData.org.id, MODULE_KEY)
  if (instance?.status === 'READY') return redirect('/factory')
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 py-4">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-slate-600 to-zinc-700 p-8 text-white shadow-md">
        ...
        <div className="relative">
          ...
          <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-300 mb-2">Modul Baru</div>
          <h1 className="text-2xl font-semibold tracking-tight">Selamat datang di Manufaktur</h1>
          <p className="mt-2 text-sm text-slate-300 leading-relaxed">Bill of Materials, work orders, konversi bahan baku jadi produk jadi, dan biaya produksi — siapkan pengaturan awal.</p>
        </div>
      </div>
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="space-y-4">
          <StepRow number={1} icon={<Zap className="h-5 w-5" />} title="Modul Diaktifkan" description="Manufaktur sudah terdaftar untuk organisasi Anda." done={stepsDone.activate} />
          <StepRow number={2} icon={<Settings className="h-5 w-5" />} title="Pengaturan Produksi" description="Tentukan satuan produksi dan kategori BoM default." done={stepsDone.settings}>
            {!stepsDone.settings && <SimpleSettingsForm moduleKey={MODULE_KEY} fields={SETTINGS_FIELDS} defaultValues={currentSettings} buttonLabel="Simpan Pengaturan Produksi" successMessage="Pengaturan produksi berhasil disimpan!" />}
          </StepRow>
        </div>
      </div>
      {allDone && <CompleteOnboardingButton moduleKey={MODULE_KEY} redirectTo="/factory" label="Mulai Produksi" />}
    </div>
  )
}
