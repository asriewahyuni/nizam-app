    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-1">
        <div className="space-y-2">
          <div className="inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-blue-700">
        ...
        <div className="flex flex-wrap gap-3 w-full md:w-auto">
          ...
          <Link
            href="/lms/admin"
            className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-slate-900 text-white text-sm font-bold rounded-xl hover:bg-slate-800 transition-all shadow-xl shadow-slate-200"
          >
      ...
      <div className="grid gap-6 xl:grid-cols-[1.5fr_1fr]">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between gap-4 mb-6">
            <div>
              ...
              <h2 className="mt-2 text-xl font-semibold text-slate-900">Batch Berjalan (Open)</h2>
            </div>
            <Link
              href="/lms/admin"
          ...
          <div className="space-y-4">
            {openBatches.length === 0 ? (
              ...
              openBatches.map((b: any) => (
                <div key={b.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-xl border border-slate-100 hover:border-blue-100 bg-slate-50 hover:bg-blue-50/30 transition-colors group">
                  <div className="flex gap-4 items-center">
                    ...
                    <div>
                      <h3 className="text-base font-semibold text-slate-900 group-hover:text-blue-700 transition-colors">{b.name}</h3>
                  ...
                  <div className="flex items-center gap-6 text-sm">
                    <div className="text-right">
                      <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Harga</div>
                      <div className="font-bold text-slate-700 mt-0.5">{b.price === 0 ? 'GRATIS' : formatRupiah(b.price)}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Kuota</div>
        ...
        <section className="rounded-xl border border-slate-200 bg-slate-900 p-6 shadow-xl relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500 rounded-full blur-3xl opacity-20 -mr-20 -mt-20" />
          
          <div className="relative">
            <div className="text-[11px] font-semibold uppercase tracking-wide text-blue-400">
              Katalog
            </div>
            ...
            <div className="mt-6 space-y-3">
              {activeCourses.slice(0, 4).map((c: any) => (
                <div key={c.slug} className="flex items-center gap-3 p-4 rounded-xl bg-white/10 hover:bg-white/15 transition-colors border border-white/5">
                  <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center text-blue-300">
                    <BookOpen size={18} />
                  </div>
                  <div>
                    <h3 className="text-sm font-bold text-white">{c.title}</h3>
                    <div className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide mt-1">Level: {c.level_code || 'ALL'}</div>
                  </div>
                </div>
              ))}
