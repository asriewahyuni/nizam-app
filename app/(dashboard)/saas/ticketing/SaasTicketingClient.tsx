...
import { ArrowUpRight, CheckCircle2, Clock3, Eye, EyeOff, LifeBuoy, Send } from 'lucide-react'
import {
...
export default function SaasTicketingClient({ snapshot }: SaasTicketingClientProps) {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>, ticketId: string) => {
    ...
    startTransition(async () => {
      ...
      setMessage({ type: 'ok', text: 'Progress tiket berhasil disimpan.' })
      formEl.reset()
      router.refresh()
    })
  }
    ...
    <div className="space-y-6 pb-16">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em] text-indigo-700">
              <LifeBuoy size={14} />
              SaaS Operator Ticketing
            </div>
            <h1 className="mt-3 text-2xl font-black text-slate-900 md:text-3xl">Progress Update Bug Report</h1>
            <p className="mt-2 text-sm font-medium text-slate-600">
              Kelola progres ticket user dan publikasikan update yang akan tampil pada halaman doc update pengguna.
            </p>
          </div>
          <Link
      ...
      {message && (
        <div className={`rounded-2xl border px-4 py-3 text-sm font-bold ${message.type === 'ok' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-rose-200 bg-rose-50 text-rose-700'}`}>
          {message.text}
        </div>
      )}
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <div className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">Total Ticket</div>
      ...
      <section className="space-y-4">
        {snapshot.tickets.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-5 py-6 text-sm font-semibold text-slate-500">
            ...
            return (
              <article key={ticket.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className={`rounded-full border px-2 py-1 text-[10px] font-black uppercase tracking-[0.12em] ${severityBadge(ticket.severity)}`}>
                    {ticket.severity}
                  </span>
                  <span className={`rounded-full border px-2 py-1 text-[10px] font-black uppercase tracking-[0.12em] ${statusBadge(ticket.status)}`}>
                    {ticket.status}
                  </span>
                  <span className="rounded-full border border-slate-200 bg-white px-2 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-slate-500">
                    {ticket.organization?.name || 'Tanpa Nama Org'}
                  </span>
                </div>
                <h3 className="mt-3 text-base font-black text-slate-900">{ticket.title}</h3>
                ...
                {ticket.screenshot_url ? (
                  <a href={ticket.screenshot_url} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-xs font-black text-blue-600 hover:text-blue-700">
                    <Eye size={12} />
                    Lihat Screenshot
                  </a>
                ) : null}
                ...
                {lastUpdate ? (
                  <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-3">
                    <div className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">Update Terakhir</div>
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-[10px] font-black uppercase tracking-[0.12em] text-slate-500">
                ...
                <form onSubmit={(event) => handleSubmit(event, ticket.id)} className="mt-4 grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 md:grid-cols-2">
                  <label className="block">
                    <span className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-500">Judul Update</span>
                    <input
                      required
                      name="update_title"
                  ...
                  <label className="block">
                    <span className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-500">Status Setelah Update</span>
                    <select
                      name="status_after"
                  ...
                  <label className="block md:col-span-2">
                    <span className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-500">Detail Update</span>
                    <textarea
                      name="update_body"
                      rows={3}
                  ...
                  <div className="md:col-span-2">
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-white hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60"
                    >
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5">
        <h2 className="text-sm font-black uppercase tracking-[0.14em] text-slate-600">Feed Update Terbaru</h2>
        <div className="mt-4 space-y-2">
          {snapshot.updates.length === 0 ? (
            ...
            snapshot.updates.slice(0, 20).map((update) => (
              <div key={update.id} className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs">
                <div className="flex flex-wrap items-center gap-2 font-black text-slate-700">
                  <span>{ticketNoById.get(update.ticket_id) || 'Tiket'}</span>
