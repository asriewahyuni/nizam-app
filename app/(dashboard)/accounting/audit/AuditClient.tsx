    ...
    <div className="max-w-7xl mx-auto space-y-12 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        ...
        <button 
          ...
          disabled={loading}
          className="flex items-center gap-3 px-8 py-5 bg-white border-2 border-slate-100 hover:border-indigo-200 hover:bg-slate-50 text-slate-900 font-black rounded-3xl transition-all shadow-sm active:scale-95 disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
         ...
         <div className={`p-8 rounded-[40px] border-2 transition-all shadow-sm flex flex-col justify-between h-full bg-white ${stats.unbalancedCount > 0 ? 'border-rose-100 shadow-rose-50' : 'border-emerald-100 shadow-emerald-50'}`}>
            ...
            <div className="mt-8 flex items-end justify-between">
               <span className={`text-5xl font-black ${stats.unbalancedCount > 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
                  {stats.unbalancedCount}
               </span>
               {stats.unbalancedCount > 0 ? (
                 <div className="flex items-center gap-2 text-rose-500 font-black text-xs uppercase animate-pulse">
         ...
         <div className={`p-8 rounded-[40px] border-2 transition-all shadow-sm flex flex-col justify-between h-full bg-white ${stats.overdueAssetCount > 0 ? 'border-indigo-100 shadow-indigo-50' : 'border-emerald-100 shadow-emerald-50'}`}>
            ...
            <div className="mt-8 flex items-end justify-between">
               <span className={`text-5xl font-black ${stats.overdueAssetCount > 0 ? 'text-indigo-600' : 'text-emerald-600'}`}>
               ...
               {stats.overdueAssetCount > 0 && (
                  <button 
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-[10px] font-black uppercase rounded-xl hover:bg-indigo-700 transition-all active:scale-95"
                  >
         ...
         <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
            <div className="p-8 border-b border-slate-50 bg-slate-50/30 flex items-center justify-between">
               <h3 className="font-black text-slate-900 text-xl flex items-center gap-3">
                  ...
                  Jurnal Bermasalah
                  {data.unbalanced.length > 0 && <span className="px-3 py-1 bg-rose-100 text-rose-600 rounded-full text-[10px] font-black">{data.unbalanced.length}</span>}
               </h3>
            </div>
                        ...
                        {data.unbalanced.map((j: any) => (
                           <tr key={j.entry_id} className="hover:bg-slate-50/50 transition-colors">
                              <td className="px-8 py-5">
                                 <p className="font-black text-slate-900 text-xs truncate max-w-[200px]">{j.description}</p>
                              ...
                              <td className="px-6 py-5 text-right font-bold text-slate-700 text-xs">{formatCurrency(j.total_credit)}</td>
                               <td className="px-8 py-5 text-right font-black text-rose-600 text-xs">{formatCurrency(j.diff)}</td>
         ...
         <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
            <div className="p-8 border-b border-slate-50 bg-slate-50/30 flex items-center justify-between">
               <h3 className="font-black text-slate-900 text-xl flex items-center gap-3">
                  ...
                  {data.overdueAssets.length > 0 && <span className="px-3 py-1 bg-indigo-100 text-indigo-600 rounded-full text-[10px] font-black">{data.overdueAssets.length}</span>}
               </h3>
            </div>
            <div className="flex-1 overflow-x-auto min-h-[300px]">
                     ...
                     <tbody className="divide-y divide-slate-50 font-medium">
                        {data.overdueAssets.map((a: any) => (
                           <tr key={a.id} className="hover:bg-slate-50/50 transition-colors">
                              <td className="px-8 py-5">
                                 <p className="font-black text-slate-900 text-xs truncate max-w-[200px]">{a.name}</p>
                              ...
                              <td className="px-8 py-5 text-right font-black text-slate-900 text-xs">{formatCurrency(a.current_book_value)}</td>
                           </tr>
                        ))}
                     </tbody>
                  </table>
      ...
      <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
         <div className="p-8 border-b border-slate-50 bg-slate-50/30 flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 bg-white border-2 border-amber-100 text-amber-600 text-[10px] font-black uppercase rounded-2xl hover:bg-amber-50 transition-all active:scale-95 disabled:opacity-50"
            >
            ...
            <table className="w-full text-left">
               ...
               <tbody className="divide-y divide-slate-50 font-medium">
                  {data.inventory.length === 0 ? (
                    <tr>
                       <td colSpan={7} className="px-8 py-20 text-center opacity-30 font-black text-xs uppercase">Tidak ada data persediaan untuk diaudit.</td>
                    </tr>
                  ) : (
                    data.inventory.map((i: any) => (
                      <tr key={i.product_id} className="hover:bg-slate-50/50 transition-colors">
                         ...
                         <td className="px-6 py-6 text-right font-black text-slate-700 text-xs">{i.stock_qty}</td>
                         <td className="px-6 py-6 text-right font-bold text-slate-400 text-xs">{formatCurrency(i.avg_cost)}</td>
                         <td className="px-6 py-6 text-right font-black text-indigo-600 text-xs">{formatCurrency(i.on_hand_value)}</td>
                         <td className="px-6 py-6 text-right font-bold text-slate-400 text-xs italic opacity-60">{formatCurrency(i.ledger_value)}</td>
                          <td className={`px-8 py-6 text-right font-black text-xs ${Math.abs(i.variance) > 10 ? 'text-amber-500' : 'text-emerald-500'}`}>
