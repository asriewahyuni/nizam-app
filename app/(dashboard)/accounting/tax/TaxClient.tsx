    ...
    <motion.div variants={container} initial="hidden" animate="show" className="max-w-7xl mx-auto space-y-10 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
         <motion.div variants={item} className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
               ...
               <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Aset Pajak</span>
            </div>
         ...
         <motion.div variants={item} className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
               ...
               <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Kewajiban Pajak</span>
            </div>
            <div className="space-y-1">
         ...
         <motion.div variants={item} className="bg-slate-900 p-8 rounded-[32px] shadow-md shadow-emerald-200/20 text-white relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-4">
               <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-semibold text-white/40 uppercase tracking-wide">Kurang/Lebih Bayar</span>
               </div>
               <div className="space-y-1">
         ...
         <motion.div variants={item} className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
               ...
               <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Jatuh Tempo</span>
            </div>
            <div className="space-y-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <motion.div variants={item} className="lg:col-span-1 space-y-8">
            <div className="bg-white p-8 rounded-xl border border-slate-100 shadow-sm space-y-8 h-full">
               <div className="flex items-center justify-between">
                 <h4 className="font-semibold text-slate-900 text-sm uppercase tracking-wide">Distribusi PPN</h4>
               ...
               <div className="h-64 relative">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                     <span className="text-[10px] font-semibold text-slate-400 uppercase">Efek Kas</span>
                     <span className={`text-xs font-semibold ${summary.netVat >= 0 ? 'text-rose-500' : 'text-emerald-500'}`}>
               ...
               <div className="space-y-4">
                  {vatData.map((d) => (
                    <div key={d.name} className="flex items-center justify-between p-4 rounded-xl bg-slate-50 border border-slate-100">
                       <div className="flex items-center gap-3">
                          <div className="w-3 h-3 rounded-full" style={{ backgroundColor: d.color }} />
                          <span className="text-xs font-bold text-slate-600">{d.name}</span>
                       </div>
                       <span className="text-xs font-semibold text-slate-900">{formatRupiah(d.value)}</span>
                    </div>
                  ))}
               </div>
            </div>
                  ...
                  <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden min-h-[500px]">
                     ...
                     <div className="overflow-x-auto">
                        <table className="w-full text-left">
                           <thead className="bg-slate-50/50 text-[10px] uppercase font-semibold tracking-wide text-slate-400 border-b border-slate-50">
                              <tr>
                           ...
                           <tbody className="divide-y divide-slate-50">
                              {[...summary.vatIn.items, ...summary.vatOut.items].sort(compareTaxDateDesc).map((it, idx) => (
                                <tr key={idx} className="hover:bg-slate-50 transition-colors group">
                                   ...
                                   <td className="px-6 py-5 font-mono text-[11px] font-semibold text-slate-400">{it.ref}</td>
                                   <td className="px-6 py-5 text-xs font-bold text-slate-900 max-w-xs truncate">{it.description}</td>
                                   <td className="px-6 py-5">
                                      <span className={`px-3 py-1 rounded-full text-[9px] font-semibold ${summary.vatIn.items.includes(it) ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-rose-50 text-rose-600 border border-rose-100'}`}>
                                         {summary.vatIn.items.includes(it) ? 'PPN MASUKAN' : 'PPN KELUARAN'}
                                      </span>
                                   </td>
                                   <td className={`px-8 py-5 text-right font-semibold text-sm ${summary.vatIn.items.includes(it) ? 'text-emerald-600' : 'text-rose-600'}`}>
                ...
                >
                   ...
                   <div className="grid grid-cols-2 gap-6">
                      <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm space-y-4">
                         <div className="flex items-center gap-3">
                            ...
                            <h5 className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Hutang PPh 21</h5>
                         </div>
                      ...
                      <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm space-y-4">
                         <div className="flex items-center gap-3">
                            ...
                            <h5 className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Hutang PPh 23</h5>
                         </div>
                   ...
                   <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
                      <div className="p-8 border-b border-slate-50 bg-slate-50/30">
                         <h4 className="text-sm font-semibold text-slate-900 uppercase tracking-wide">Aktivitas Potong PPh</h4>
                      </div>
                      <div className="overflow-x-auto">
                        <table className="w-full text-left">
                           <thead className="bg-slate-50/50 text-[10px] uppercase font-semibold tracking-wide text-slate-400 border-b border-slate-50">
                           ...
                           <tbody className="divide-y divide-slate-50">
                              {[...summary.pph21.items.map((i:any) => ({...i, type: 'PPh 21'})), ...summary.pph23.items.map((i:any) => ({...i, type: 'PPh 23'}))].sort(compareTaxDateDesc).map((it, idx) => (
                                <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                   ...
                                   <td className="px-6 py-5 text-xs font-semibold text-slate-400">{it.type}</td>
                                   <td className="px-8 py-5 text-right font-semibold text-slate-900">{formatRupiah(it.amount)}</td>
                                </tr>
                              ))}
