          ...
          >
              ...
              <div className="flex flex-col gap-8 md:flex-row md:items-start md:justify-between">
                <div className="flex items-start gap-4">
                  ...
                  <div className="space-y-1.5">
                    <h1 className="text-2xl font-semibold uppercase tracking-tight text-slate-900">{companyProfile.name}</h1>
                ...
                <div className="text-right">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-emerald-600">Non-Posting Collection Bill</p>
                  <h2 className="mt-2 text-4xl font-semibold tracking-tight text-slate-900">QUICK BILL</h2>
                  <p className="mt-2 text-sm font-semibold text-slate-700">{snapshot.docNumber}</p>
            ...
            <section className="grid gap-6 border-b border-slate-200 px-10 py-6 md:grid-cols-[1.2fr_0.8fr]">
              <div className="space-y-3">
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Tagihan Untuk</p>
                  <p className="mt-1 text-xl font-semibold text-slate-900">{snapshot.customer.name}</p>
                ...
                <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-amber-700">Catatan</p>
              ...
              <div className="space-y-3">
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-5">
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Grand Total Outstanding</p>
                  <p className="mt-2 text-3xl font-semibold tracking-tight text-slate-900">{formatRupiah(snapshot.totals.totalOutstanding)}</p>
                  <div className="mt-4 grid grid-cols-2 gap-3 text-xs font-semibold text-slate-500">
                    <div>
                      ...
                      <p className="mt-1 text-sm font-semibold text-slate-900">{snapshot.totals.invoiceCount}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Overdue</p>
                      <p className="mt-1 text-sm font-semibold text-slate-900">{snapshot.totals.overdueInvoiceCount}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Jatuh Tempo Terlama</p>
                      <p className="mt-1 text-sm font-semibold text-slate-900">{snapshot.totals.oldestDueDate ? formatDate(snapshot.totals.oldestDueDate) : '-'}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Maks. Keterlambatan</p>
                ...
                <div className="rounded-xl border border-slate-200 p-5">
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Komposisi Aging</p>
                  <div className="mt-3 space-y-3">
                    {snapshot.bucketBreakdown.map((item) => (
                      <div key={item.bucket} className="flex items-center justify-between gap-3 text-xs font-semibold">
                        <span className={`inline-flex rounded-full border px-3 py-1 text-[10px] font-semibold uppercase ${highlightClass(item.bucket)}`}>
                          {item.bucket}
                        </span>
            ...
            <section className="px-10 py-6">
              <div className="mb-4 flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold uppercase tracking-wide text-slate-900">Rincian Invoice Outstanding</p>
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="border-b border-slate-300 text-left">
                      <th className="py-3 text-[10px] font-semibold uppercase tracking-wide text-slate-500">No. Dokumen</th>
                      <th className="py-3 text-[10px] font-semibold uppercase tracking-wide text-slate-500">Jatuh Tempo</th>
                      <th className="py-3 text-right text-[10px] font-semibold uppercase tracking-wide text-slate-500">Nilai Invoice</th>
                      <th className="py-3 text-right text-[10px] font-semibold uppercase tracking-wide text-slate-500">Sudah Bayar</th>
                      <th className="py-3 text-right text-[10px] font-semibold uppercase tracking-wide text-slate-500">Retur</th>
                      <th className="py-3 text-right text-[10px] font-semibold uppercase tracking-wide text-slate-500">Outstanding</th>
                      <th className="py-3 text-center text-[10px] font-semibold uppercase tracking-wide text-slate-500">Aging</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={invoice.id} className="border-b border-slate-100 align-top">
                        <td className="py-4">
                          <div className="space-y-1">
                            <p className="font-semibold text-slate-900">{invoice.doc_number}</p>
                        ...
                        <td className="py-4 text-right text-base font-semibold tracking-tight text-slate-900">{formatRupiah(invoice.outstanding)}</td>
                        <td className="py-4 text-center">
                          <span className={`inline-flex rounded-full border px-3 py-1 text-[10px] font-semibold uppercase ${highlightClass(invoice.aging_bucket)}`}>
            ...
            <section className="border-t border-slate-200 bg-slate-50 px-10 py-6">
              <div className="mb-4">
                <p className="text-sm font-semibold uppercase tracking-wide text-slate-900">Lampiran Dokumen</p>
              ...
              <div className="grid gap-3">
                {snapshot.invoices.map((invoice, index) => (
                  <div key={`${invoice.id}-attachment`} className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 md:flex-row md:items-center md:justify-between">
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-[0.25em] text-slate-400">Lampiran {index + 1}</p>
                      <p className="mt-1 text-sm font-semibold text-slate-900">{invoice.doc_number}</p>
                    ...
                    {invoice.doc_href && (
                      <Link
                        href={invoice.doc_href}
                        target="_blank"
                        className="quick-bill-no-print inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-[10px] font-semibold uppercase text-slate-700 transition-colors hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700"
                      >
                        Buka Lampiran
                      </Link>
                    )}
