...
export default function InvoicePrintPage() {
  ...
  if (loading) return <div className="p-6 text-center text-slate-400 font-bold animate-pulse">Memuat Invoice...</div>
  if (!invoice) return <div className="p-6 text-center text-rose-500 font-bold italic">Invoice Tidak Ditemukan.</div>
      ...
      <div className="min-h-screen bg-slate-50 py-10 px-4 print:bg-white print:p-0">
        <div className="max-w-4xl mx-auto space-y-5">
          <div className="flex items-center justify-between invoice-no-print">
            <button type="button" onClick={() => router.back()} className="flex items-center gap-2 text-slate-500 hover:text-slate-900 font-bold transition-all">
              <ArrowLeft size={18} /> Kembali
            </button>
            <button type="button" onClick={handlePrint} className="flex items-center gap-2 px-6 py-2.5 bg-slate-900 text-white rounded-xl font-semibold text-xs uppercase tracking-wide shadow-xl hover:bg-slate-800 transition-all">
          ...
          <article id="invoice-card" className="bg-white rounded-[32px] border border-slate-200 shadow-md overflow-hidden print:rounded-none print:border-none print:shadow-none print:text-[11px]">
            <header className="px-10 py-8 border-b-2 border-slate-900">
              <div className="flex items-start justify-between gap-8">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm font-semibold text-slate-700 mt-2">{invoice.invoice_number}</p>
            ...
            <section className="px-10 py-6 border-b border-slate-200 grid grid-cols-2 gap-10">
              ...
              <div className="text-right">
                <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-2">Status</p>
                <span className={`inline-flex px-3 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wide border ${invoice.status === 'PAID' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-slate-300">
                    <th className="py-3 text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wide">Deskripsi</th>
                    <th className="py-3 text-right text-[10px] font-semibold text-slate-500 uppercase tracking-wide w-24">Qty</th>
                    <th className="py-3 text-right text-[10px] font-semibold text-slate-500 uppercase tracking-wide w-56">Total</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className="border-b border-slate-100">
                    <td className="py-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-slate-900">{invoice.item_name || 'Layanan Berlangganan'}</p>
                          <p className="text-xs text-slate-500">SaaS Subscription</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-5 text-right font-bold text-slate-700">1</td>
                    <td className="py-5 text-right font-semibold text-slate-900">{formatRupiah(invoice.amount)}</td>
                  </tr>
                </tbody>
              </table>
            </section>
            ...
            <section className="px-10 py-6 bg-slate-50 border-t border-slate-200 grid grid-cols-[1.2fr_0.8fr] gap-8">
              <div>
                <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide flex items-center gap-2 mb-3">
                  <CreditCard size={12} /> Pembayaran Transfer
                </p>
                <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-1">
                  <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">{bank.bank || '-'}</p>
                ...
                {invoiceNote && (
                  <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
                    <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Catatan</p>
              ...
              <div className="space-y-2 self-end">
                ...
                <div className="border-t border-slate-300 pt-2 flex items-center justify-between">
                  <span className="text-sm font-semibold text-slate-900 uppercase tracking-wider">Grand Total</span>
