...
import { CheckCircle2, CreditCard, Settings, Zap } from 'lucide-react'
...
const SETTINGS_FIELDS: SettingsField[] = [
  { name: 'default_payment', label: 'Metode Pembayaran Default', type: 'select', options: [{ value: 'CASH', label: 'Tunai' }, { value: 'DEBIT', label: 'Debit/Kartu' }, { value: 'QRIS', label: 'QRIS' }, { value: 'CASH_DEBIT', label: 'Tunai & Debit' }], defaultValue: 'CASH' },
  { name: 'enable_shift', label: 'Management Shift Kasir', type: 'select', options: [{ value: 'yes', label: 'Ya, aktifkan' }, { value: 'no', label: 'Tidak dulu' }], defaultValue: 'yes' },
]
...
export default async function POSOnboardingPage() {
  ...
  if (instance?.status === 'READY') return redirect('/pos')
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 py-4">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-rose-600 to-pink-700 p-8 text-white shadow-md">
        ...
        <div className="relative">
          ...
          <div className="text-[10px] font-semibold uppercase tracking-wide text-rose-200 mb-2">Add-on Baru</div>
          <h1 className="text-2xl font-semibold tracking-tight">Selamat datang di POS (Kasir)</h1>
          <p className="mt-2 text-sm text-rose-100 leading-relaxed">Kasir cepat untuk transaksi ritel dan F&B. Atur metode bayar dan shift kasir.</p>
        </div>
      </div>
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="space-y-4">
          <StepRow number={1} icon={<Zap className="h-5 w-5" />} title="Add-on Diaktifkan" description="POS sudah terdaftar untuk organisasi Anda." done={stepsDone.activate} />
          <StepRow number={2} icon={<Settings className="h-5 w-5" />} title="Pengaturan POS" description="Pilih metode bayar default dan aktifkan shift kasir." done={stepsDone.settings}>
            {!stepsDone.settings && <SimpleSettingsForm moduleKey={MODULE_KEY} fields={SETTINGS_FIELDS} defaultValues={currentSettings} buttonLabel="Simpan Pengaturan POS" successMessage="Pengaturan POS berhasil disimpan!" />}
          </StepRow>
        </div>
      </div>
      {allDone && <CompleteOnboardingButton moduleKey={MODULE_KEY} redirectTo="/pos" label="Mulai Bertransaksi" />}
    </div>
  )
}
