...
import { Bug, Camera, CircleAlert, Clock3, LifeBuoy, Send } from 'lucide-react'
...
export default function TicketingClient({ tickets }: TicketingClientProps) {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      setSelectedFileName('')
      setMessage({ type: 'ok', text: 'Tiket berhasil dikirim. Tim support akan menindaklanjuti.' })
      router.refresh()
    })
  }
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <h1 className="mt-3 text-2xl font-black text-slate-900 md:text-3xl">Laporkan Bug Sistem</h1>
            <p className="mt-2 max-w-3xl text-sm font-medium leading-relaxed text-slate-600">
              Isi detail bug sejelas mungkin, termasuk menu tempat bug muncul, kapan kejadian terjadi, dan screenshot
              agar proses investigasi lebih cepat.
            </p>
          </div>
          <Link
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
        <h2 className="text-sm font-black uppercase tracking-[0.16em] text-slate-600">Form Tiket Baru</h2>
        <form ref={formRef} onSubmit={handleSubmit} className="mt-5 grid gap-4 md:grid-cols-2">
          <label className="block">
            <span className="text-[11px] font-black uppercase tracking-[0.12em] text-slate-500">Judul Bug</span>
            <input
              required
              name="title"
          ...
          <label className="block">
            <span className="text-[11px] font-black uppercase tracking-[0.12em] text-slate-500">Tingkat Prioritas</span>
            <select
              name="severity"
              defaultValue="MEDIUM"
              className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-900 outline-none transition focus:border-blue-500"
          ...
          <label className="block">
            <span className="text-[11px] font-black uppercase tracking-[0.12em] text-slate-500">Menu/Lokasi Bug Ditemukan</span>
            <input
              required
              list={menuDatalistId}
              name="found_in_menu"
          ...
          <label className="block">
            <span className="text-[11px] font-black uppercase tracking-[0.12em] text-slate-500">Kapan Bug Terjadi</span>
            <input
              type="datetime-local"
              name="found_at"
              className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-900 outline-none transition focus:border-blue-500"
          ...
          <label className="block md:col-span-2">
            <span className="text-[11px] font-black uppercase tracking-[0.12em] text-slate-500">Pada Saat Apa Terjadi (Langkah)</span>
            <input
              name="found_during"
          ...
          <label className="block md:col-span-2">
            <span className="text-[11px] font-black uppercase tracking-[0.12em] text-slate-500">Deskripsi Detail Bug</span>
            <textarea
              required
              name="description"
              rows={5}
          ...
          <label className="block md:col-span-2">
            <span className="inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.12em] text-slate-500">
              <Camera size={14} />
            ...
            <input
              ...
              className="mt-2 block w-full rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700"
            />
          ...
          {message && (
            <div
              className={`md:col-span-2 rounded-2xl border px-4 py-3 text-sm font-bold ${
                message.type === 'ok'
          ...
          <div className="md:col-span-2">
            ...
            >
              <Send size={15} />
              {isPending ? 'Mengirim Tiket...' : 'Kirim Tiket'}
            </button>
          </div>
        </form>
      </section>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
        <h2 className="text-sm font-black uppercase tracking-[0.16em] text-slate-600">Riwayat Tiket</h2>
        <div className="mt-5 space-y-3">
          {tickets.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-5 text-sm font-semibold text-slate-500">
              Belum ada tiket. Silakan kirim laporan bug pertama Anda.
            </div>
          ) : (
            tickets.map((ticket) => (
              <article key={ticket.id} className="rounded-2xl border border-slate-200 bg-white p-4">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className={`rounded-full border px-2 py-1 text-[10px] font-black uppercase tracking-[0.12em] ${severityClassName(ticket.severity)}`}>
                    {ticket.severity}
                  </span>
                  <span className={`rounded-full border px-2 py-1 text-[10px] font-black uppercase tracking-[0.12em] ${statusClassName(ticket.status)}`}>
                    {ticket.status}
                  </span>
                </div>
                <h3 className="mt-3 text-sm font-black text-slate-900">{ticket.title}</h3>
                ...
                {ticket.screenshot_url ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-3 inline-flex items-center gap-1 text-xs font-black text-blue-600 hover:text-blue-700"
                  >
                    <CircleAlert size={13} />
                    Lihat screenshot
                  </a>
