...
export default function RolesManagementPage() {
  ...
  if (loading && roles.length === 0) return <div className="p-20 text-center animate-pulse text-slate-400 font-semibold uppercase tracking-wide text-[10px]">Menyinkronkan Struktur...</div>

  return (
    <div className="flex h-[calc(100vh-140px)] overflow-hidden bg-slate-50/50 rounded-xl p-2 border border-slate-100 shadow-md relative">
      ...
      {/* SIDEBAR */}
      <aside className="w-80 flex flex-col bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-8 border-b border-slate-50">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-xs font-semibold text-slate-900 uppercase tracking-wide leading-none">Hak Akses</h2>
              <p className="text-[10px] text-slate-400 font-bold mt-1">Struktur Jabatan</p>
            </div>
          </div>
          <SafeButton variant="primary" icon={<Plus size={14} />} onClick={() => setNewRoleModal(true)} className="w-full py-4 text-[10px] tracking-wide font-semibold uppercase">
            TAMBAH JABATAN
          </SafeButton>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
          {roles.length === 0 && !loading && (
            <div className="p-5 text-center space-y-3 opacity-30">
              <Users size={32} className="mx-auto" />
              <p className="text-[10px] font-semibold uppercase tracking-wide">Jabatan Kosong</p>
            </div>
          )}
      ...
      {/* DETAIL PANEL */}
      <main className="flex-1 flex flex-col h-full bg-white rounded-xl ml-2 border border-slate-100 relative overflow-hidden">
        <AnimatePresence mode="wait">
          {activeRole ? (
            <motion.div key={activeRole.id} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="h-full flex flex-col">
              <header className="p-8 border-b border-slate-50 flex items-center justify-between bg-white z-10 shadow-sm">
                <div className="flex items-center gap-6">
                  <div className="w-16 h-16 bg-slate-50 rounded-xl flex items-center justify-center text-slate-400 border border-slate-200 shadow-inner">
                    <Users size={28} />
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                       {activeRoleDepartmentIds.map((d: string) => (
                         <span key={d} className="px-2 py-0.5 bg-indigo-50 text-indigo-600 border border-indigo-100 text-[8px] font-semibold rounded uppercase tracking-wide">{d}</span>
                       ))}
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 custom-scrollbar pb-32">
                {activeCategories.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center space-y-6 max-w-md mx-auto py-20">
                     <div className="w-24 h-24 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center shadow-md shadow-indigo-100">
                        <Sparkles size={40} />
                     </div>
                     <div>
                      ...
                      <div key={cat.category} className="space-y-6">
                          <div className="flex items-center justify-between border-b-2 border-slate-50 pb-3 h-10">
                            <div className="flex items-center gap-3">
                                ...
                                <h4 className="text-[12px] font-semibold text-slate-900 uppercase tracking-wide">{cat.category}</h4>
                            </div>
                          </div>
                          <div className="grid grid-cols-1 gap-3">
                            {cat.modules.map(mod => {
                              ...
                              return (
                                <div key={mod.id} className="flex items-center justify-between p-5 bg-white border border-slate-100 rounded-xl group hover:border-indigo-100 hover:bg-indigo-50/20 transition-all shadow-sm">
                                    <div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-300 gap-4">
              <div className="w-24 h-24 bg-slate-50 rounded-xl flex items-center justify-center border border-slate-100 shadow-inner">
                <Shield size={40} className="opacity-20" />
              </div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.3em]">Pilih Jabatan untuk Mengatur Izin</p>
            </div>
          )}
        </AnimatePresence>
      </main>
      ...
      <AnimatePresence>
         {newRoleModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
                ...
                <motion.div initial={{ opacity: 0, scale: 0.9, y: 40 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 40 }} className="relative bg-white rounded-[60px] shadow-md w-full max-w-xl p-16 border border-white max-h-[90vh] overflow-y-auto custom-scrollbar">
                   ...
                   <form onSubmit={submitRole} className="space-y-10">
                     <div className="space-y-3">
                       <label className="text-[10px] font-semibold uppercase text-slate-400 ml-2 tracking-[0.3em]">Nama Jabatan</label>
                       <input autoFocus value={newRoleName} onChange={e => setNewRoleName(e.target.value)} required className="w-full px-10 py-6 bg-slate-50 border-2 border-slate-100 rounded-xl text-sm font-semibold transition-all" placeholder="Cth: Kepala Gudang Pusat" />
                     </div>
                     <div className="space-y-3">
                       <label className="text-[10px] font-semibold uppercase text-slate-400 ml-2 tracking-[0.3em]">Jabatan Atasan Langsung</label>
                       <select value={newRoleParent || ''} onChange={e => setNewRoleParent(e.target.value || null)} className="w-full px-10 py-6 bg-slate-50 border-2 border-slate-100 rounded-xl text-sm font-semibold appearance-none">
                     ...
                     <div className="space-y-4">
                        <label className="text-[10px] font-semibold uppercase text-slate-400 ml-2 tracking-[0.3em]">Klasifikasi Departemen</label>
                        <div className="grid grid-cols-2 gap-4">
                          {MODULE_CATEGORIES.map(cat => (
                             ...
                             }} className={`p-6 rounded-[32px] text-[10px] font-semibold uppercase tracking-tight text-center transition-all border-2 flex flex-col items-center justify-center gap-2 ${newRoleDepts.includes(cat.val) ? 'bg-indigo-600 text-white border-indigo-600 shadow-md shadow-indigo-200' : 'bg-slate-50 text-slate-300 border-slate-100 hover:border-slate-300'}`}>
                               {cat.category}
                             </button>
                          ))}
                        </div>
                     ...
                     <div className="pt-8 flex gap-4">
                        <button type="button" onClick={resetModal} className="flex-1 py-6 bg-slate-100 text-slate-500 rounded-xl font-semibold text-[11px] uppercase tracking-wide hover:bg-slate-200 transition-colors">Batal</button>
                        <button type="submit" className="flex-1 py-6 bg-indigo-600 text-white rounded-xl font-semibold text-[11px] uppercase tracking-wide hover:bg-indigo-700 transition-all shadow-md shadow-indigo-200">{editingInfo ? 'SIMPAN PERUBAHAN' : 'BUAT JABATAN'}</button>
                     </div>
                  </form>
                </motion.div>
            </div>
