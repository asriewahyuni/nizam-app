  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-semibold text-slate-900 tracking-tight flex items-center gap-3">
            <div className="p-3 bg-[#003366] text-white rounded-xl shadow-lg shadow-blue-900/20">
              <ShieldCheck size={28} strokeWidth={2.5} />
            </div>
            DASHBOARD AUDIT SISTEM
          </h1>
          <p className="text-sm text-slate-500 font-medium ml-16 mt-1">
            Log Forensik Real-time & Metadata Transaksi <span className="text-blue-600 font-semibold uppercase">{orgName}</span>
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
            <button 
              ...
              className="px-6 py-4 bg-rose-50 hover:bg-rose-100 text-rose-600 font-semibold rounded-xl transition-all flex items-center gap-2 border border-rose-200"
            >
              <Zap size={20} />
              RESET TRANSAKSI
            </button>
            ...
            <button 
              ...
              className="px-6 py-4 bg-slate-900 hover:bg-black text-white font-semibold rounded-xl transition-all shadow-xl active:scale-95 flex items-center gap-2"
            >
              <Building2 size={20} />
              WIPE ALL
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-[#003366] rounded-xl p-6 text-white shadow-xl shadow-blue-900/20 relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-8 opacity-10 transform translate-x-4 -translate-y-4 group-hover:scale-110 transition-transform">
            <Activity size={100} />
          </div>
          <p className="text-[10px] font-semibold text-blue-300 uppercase tracking-wide mb-1">TOTAL AKTIVITAS</p>
          <h2 className="text-4xl font-semibold tracking-tighter mb-4">{logs.length}+</h2>
          <div className="inline-flex items-center gap-2 bg-white/10 px-3 py-1 rounded-full border border-white/20">
            <div className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse" />
            <span className="text-[9px] font-semibold tracking-wide">LIVE CCTV ACTIVE</span>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col justify-between">
          <div className="flex justify-between items-start">
             ...
             <span className="text-[10px] font-semibold text-emerald-500 uppercase">ISO:27001 Compliant</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-slate-800 tracking-tight uppercase">Immutable Logs</h3>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col justify-between">
          <div className="flex justify-between items-start">
             <div className="p-3 bg-rose-50 text-rose-600 rounded-xl"><Eye size={20} /></div>
             <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Level 1 Awareness</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-slate-800 tracking-tight uppercase">Cyber Forensic Ready</h3>
          ...
          <div className="p-6 bg-slate-50/50 border-b border-slate-100 flex flex-col md:flex-row md:items-center gap-4">
             <div className="relative flex-1">
                ...
                <input 
                  ...
                  className="w-full bg-white border border-slate-200 rounded-xl py-3 pl-12 pr-4 text-xs font-bold focus:ring-2 focus:ring-[#003366] focus:border-transparent outline-none shadow-sm"
                />
             </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
            {filteredLogs.map(log => (
              <div 
                ...
                className={cn(
                  "p-4 rounded-xl border transition-all cursor-pointer group flex items-start gap-4 h-fit",
              ...
              >
                ...
                <div className="flex-1 space-y-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={cn("text-[9px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded border", getActionStyles(log.action))}>{log.action}</span>
                    <span className="text-[9px] font-semibold text-slate-400 uppercase tracking-wide bg-slate-100 px-1.5 py-0.5 rounded">{log.table_name}</span>
                  ...
                  <p className="text-xs font-semibold text-slate-800 tracking-tight leading-relaxed">{log.description}</p>
        ...
        <AnimatePresence mode="wait">
          {selectedLog ? (
            <motion.div 
              ...
              exit={{ opacity: 0, x: 20 }}
              className="lg:col-span-5 bg-slate-900 rounded-[32px] p-8 text-white h-[600px] flex flex-col shadow-md overflow-hidden border border-slate-800 relative"
            >
              <div className="flex items-center justify-between mb-8">
                 <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl bg-white/10 flex items-center justify-center text-blue-400 border border-white/10">
                       <FileCode size={24} />
                    </div>
                    <div>
                       <h3 className="text-lg font-semibold tracking-tight uppercase">Forensic Report</h3>
                 ...
                 <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                       <div className="bg-white/5 p-4 rounded-xl border border-white/5 shadow-inner">
                          <p className="text-[9px] font-semibold text-slate-500 uppercase mb-2">Operation ID</p>
                          <p className="text-[10px] font-mono font-bold text-blue-300 break-all">{selectedLog.id}</p>
                       </div>
                       <div className="bg-white/5 p-4 rounded-xl border border-white/5 shadow-inner">
                          <p className="text-[9px] font-semibold text-slate-500 uppercase mb-2">Record Target</p>
                          <p className="text-[10px] font-mono font-bold text-blue-300 break-all">{selectedLog.record_id}</p>
                       </div>
                    </div>

                    <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                        <p className="text-[9px] font-semibold text-slate-500 uppercase mb-2">User Context</p>
                        <p className="text-xs font-semibold text-white">{String(selectedLog.user_name ?? '')}</p>
                    ...
                    {selectedLog.old_data != null && (
                      <div className="space-y-2">
                        <p className="text-[10px] font-semibold text-rose-500 uppercase flex items-center gap-2"><AlertTriangle size={12}/> Original State (OLD)</p>
                        <pre className="text-[10px] font-mono p-4 bg-rose-950/50 rounded-xl border border-rose-900/30 overflow-x-auto leading-relaxed shadow-inner max-h-48 text-rose-100">
                    ...
                    {selectedLog.new_data != null && (
                      <div className="space-y-2">
                        <p className="text-[10px] font-semibold text-emerald-500 uppercase flex items-center gap-2"><ShieldCheck size={12}/> Updated State (NEW)</p>
                        <pre className="text-[10px] font-mono p-4 bg-emerald-950/50 rounded-xl border border-emerald-900/30 overflow-x-auto leading-relaxed shadow-inner max-h-48 text-emerald-100">
            ...
            <div className="lg:col-span-5 hidden lg:flex flex-col items-center justify-center p-6 bg-slate-50 rounded-[32px] border border-dashed border-slate-300 text-slate-300 space-y-4">
               <div className="w-24 h-24 rounded-full border-4 border-slate-100 flex items-center justify-center animate-pulse">
                  <ShieldCheck size={48} strokeWidth={1} />
               </div>
               <div className="text-center">
                  <p className="font-semibold uppercase text-xs tracking-wide">Inspektor Forensik</p>
