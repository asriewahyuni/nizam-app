    ...
    <div className="space-y-4">
      ...
      <SectionCard>
        {loading ? <div className="text-white/50 p-4">Memuat...</div> : data.length === 0 ? (
          <div className="text-white/50 p-8 text-center">Belum ada proyek. Buat proyek baru untuk memulai Mudharabah.</div>
        ) : (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <h3 className="text-sm font-semibold text-white">{p.nama_proyek}</h3>
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium text-white ${flow.color}`}>
                          {flow.label}
                        </span>
                      </div>
                      <p className="text-xs text-white/40 mt-1">{p.deskripsi || 'Tidak ada deskripsi'}</p>
      ...
      <Modal show={showForm} onClose={() => setShowForm(false)} title="Proyek Mudharabah Baru" size="lg">
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="p-3 bg-emerald-900/20 rounded-xl text-xs text-emerald-400">
          ...
          <div className="space-y-4">
            {investasi.length > 0 && (
              <div className="space-y-2">
                <h4 className="text-xs font-semibold text-white/60 uppercase tracking-wider">Investasi Saat Ini</h4>
                {investasi.map((inv: any) => (
                  <div key={inv.id} className="flex justify-between p-3 rounded-lg bg-white/5">
                    <span className="text-sm text-white/80">{inv.shahibul_maal?.anggota?.nama || '-'}</span>
                    <span className="text-sm text-emerald-400 font-medium">Rp {Number(inv.jumlah_setor).toLocaleString()} ({inv.porsi_persen}%)</span>
                  </div>
                ))}
              </div>
            )}
