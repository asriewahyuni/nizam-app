...
export default function AnggotaPage() {
  ...
  const loadAnggota = useCallback(async (oid: string) => {
    try {
      const d = await api('getAnggota', [oid])
      setAnggota(d)
      setLoading(false)
    } catch (e: any) {
      setError(e.message)
  ...
  useEffect(() => {
    // Fetch current orgId from a lightweight endpoint
    fetch('/api/koperasi/action', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'getAnggota', params: ['_ping'] }),
    })
    .then(res => {
      if (res.status === 401) {
      ...
      // Try to get org from another endpoint
      return fetch('/api/internal/user-info', { method: 'GET' })
    })
    .then(async (res) => {
      if (res && res.ok) {
        const data = await res.json()
        const oid = data.orgId || data.active_org_id || data.org?.id
        if (oid) {
          setOrgId(oid)
          loadAnggota(oid)
        } else {
