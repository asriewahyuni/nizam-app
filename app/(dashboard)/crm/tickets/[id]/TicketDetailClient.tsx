...
import type { CrmTicket, CrmTicketNote, CrmTicketStatus, CrmTicketPriority } from '@/modules/crm/lib/ticket-constants'
import { TICKET_TYPE_LABEL, TICKET_PRIORITY_LABEL } from '@/modules/crm/lib/ticket-constants'
import { updateCrmTicket, addCrmTicketNote, getCrmTicket } from '@/modules/crm/actions/tickets.actions'
...
interface Props {
  ticket: CrmTicket
  notes: CrmTicketNote[]
  assignableStaff: { id: string; display_name: string; login_email: string }[]
  orgId: string
...
export function TicketDetailClient({ ticket: initialTicket, notes: initialNotes, assignableStaff, orgId, currentUserName }: Props) {
  ...
  const handleStatusChange = (newStatus: CrmTicketStatus) => {
    ...
    startTransition(async () => {
      ...
      // Fetch new notes generated by trigger
      const updatedData = await getCrmTicket(ticket.id, orgId)
      if (updatedData) {
  ...
  const handleAssigneeChange = (newAssigneeId: string) => {
    const assigned_to_user_id = newAssigneeId === 'UNASSIGNED' ? null : newAssigneeId
    startTransition(async () => {
      const result = await updateCrmTicket(ticket.id, { assigned_to_user_id })
      if ('error' in result) { alert(result.error); return }
      const chosenStaff = assignableStaff.find(s => s.id === assigned_to_user_id)
      setTicket(prev => ({
        ...prev,
        assigned_to_user_id,
        assigned_to_name: chosenStaff ? chosenStaff.display_name : null,
      }))
      // Fetch new notes generated by trigger
      const updatedData = await getCrmTicket(ticket.id, orgId)
      if (updatedData) {
  ...
  const handlePriorityChange = (newPriority: CrmTicketPriority) => {
    if (newPriority === ticket.priority) return
    startTransition(async () => {
      const result = await updateCrmTicket(ticket.id, { priority: newPriority })
      if ('error' in result) { alert(result.error); return }
      setTicket(prev => ({
        ...prev,
          ...
          <div className="md:col-span-2 space-y-5">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              ...
              <div className="space-y-3 mb-4">
                ...
                {notes.map(note => {
                  if (note.author_type === 'SYSTEM') {
                    return (
                      <div key={note.id} className="flex items-start gap-2.5 text-xs text-slate-500 py-1.5 pl-3 border-l-2 border-slate-200">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              ...
              <div className="space-y-2.5 text-sm">
                ...
                <Row label="Prioritas" value={
                  <select
                    value={ticket.priority}
                    onChange={e => handlePriorityChange(e.target.value as CrmTicketPriority)}
                    disabled={isPending}
                    className="text-xs border border-slate-200 rounded-lg px-2 py-1 focus:outline-none focus:ring-1 focus:ring-[#003366]/20 bg-white font-medium cursor-pointer"
                  >
                    {Object.keys(TICKET_PRIORITY_LABEL).map(p => (
                      <option key={p} value={p}>{TICKET_PRIORITY_LABEL[p as CrmTicketPriority]}</option>
                    ))}
                  </select>
                } />
                ...
                <Row label="Ditangani" value={
                  <select
                    value={ticket.assigned_to_user_id || 'UNASSIGNED'}
                    onChange={e => handleAssigneeChange(e.target.value)}
                    disabled={isPending}
                    className="text-xs border border-slate-200 rounded-lg px-2 py-1 focus:outline-none focus:ring-1 focus:ring-[#003366]/20 bg-white font-medium cursor-pointer max-w-[150px] truncate"
                  >
