    ...
    <MotionWrapper>
        ...
        <div className="mt-5 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-slate-600">
              Lesson {currentLessonIndex + 1} dari {lessons.length}
            </div>
            <h1 className="mt-4 text-3xl font-semibold tracking-tight text-slate-900 md:text-4xl">
              {lesson.title}
            </h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600 md:text-base">
              {lesson.summary || 'Pelajari materi berikut dengan saksama.'}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 md:p-8 shadow-sm">
        ...
        <div 
          ...
          dangerouslySetInnerHTML={{ __html: lesson.content_md || '<p>Tidak ada konten teks.</p>' }}
        />
      </section>
