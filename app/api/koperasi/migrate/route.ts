...
export async function GET() {
  try {
    ...
    // Create koperasi_simpanan_pokok
    await queryPostgres(`
    ...
    // Create koperasi_simpanan_wajib
    await queryPostgres(`
    ...
    // Create koperasi_simpanan_sukarela
    await queryPostgres(`
      CREATE TABLE IF NOT EXISTS koperasi_simpanan_sukarela (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        org_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
        anggota_id UUID NOT NULL REFERENCES koperasi_anggota(id) ON DELETE CASCADE,
        jenis VARCHAR(10) NOT NULL DEFAULT 'SETORAN',
        jumlah DECIMAL(18,2) NOT NULL DEFAULT 0,
    ...
    await queryPostgres(`
      CREATE TABLE IF NOT EXISTS koperasi_shahibul_maal (
        ...
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE(org_id, anggota_id)
      )
    `)

    // Create koperasi_mudharib
    await queryPostgres(`
    ...
    // Create koperasi_sertifikasi_dps
    await queryPostgres(`
    ...
    await queryPostgres(`
      CREATE TABLE IF NOT EXISTS koperasi_pengurus (
        ...
        tgl_menjabat DATE NOT NULL DEFAULT CURRENT_DATE,
        tgl_berakhir DATE,
        status VARCHAR(20) NOT NULL DEFAULT 'AKTIF',
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE(org_id, anggota_id, jabatan)
      )
    `)

    // Create koperasi_akad_wakalah
    await queryPostgres(`
      CREATE TABLE IF NOT EXISTS koperasi_akad_wakalah (
        ...
        ujrah_flat DECIMAL(5,4) NOT NULL DEFAULT 0.02,
        total_ujrah DECIMAL(18,2) NOT NULL,
        total_pembayaran DECIMAL(18,2) NOT NULL,
        jangka_wakalah INT NOT NULL DEFAULT 1,
        status VARCHAR(20) NOT NULL DEFAULT 'AKTIF',
        tgl_akad DATE NOT NULL DEFAULT CURRENT_DATE,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE(org_id, nomor_akad)
      )
    `)

    // Create koperasi_murabahah_transaksi
    await queryPostgres(`
      CREATE TABLE IF NOT EXISTS koperasi_murabahah_transaksi (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        org_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
        akad_id UUID NOT NULL REFERENCES koperasi_akad_wakalah(id) ON DELETE CASCADE,
        ...
        sisa_angsuran DECIMAL(18,2) NOT NULL DEFAULT 0,
        jumlah_angsuran INT NOT NULL DEFAULT 1,
    ...
    await queryPostgres(`
      CREATE TABLE IF NOT EXISTS koperasi_proyek (
        ...
        nama VARCHAR(255) NOT NULL,
        deskripsi TEXT,
        modal_dibutuhkan DECIMAL(18,2) NOT NULL DEFAULT 0,
        modal_terkumpul DECIMAL(18,2) NOT NULL DEFAULT 0,
        nisbah_mudharib DECIMAL(5,4) NOT NULL DEFAULT 0.50,
    ...
    // Create koperasi_investasi_proyek
    await queryPostgres(`
      CREATE TABLE IF NOT EXISTS koperasi_investasi_proyek (
        ...
        tgl_investasi DATE NOT NULL DEFAULT CURRENT_DATE,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE(proyek_id, shahibul_maal_id)
      )
    `)

    // Create koperasi_bagi_hasil
    await queryPostgres(`
      CREATE TABLE IF NOT EXISTS koperasi_bagi_hasil (
        ...
        tgl_hitung DATE NOT NULL DEFAULT CURRENT_DATE,
        total_laba DECIMAL(18,2) NOT NULL DEFAULT 0,
        bagian_mudharib DECIMAL(18,2) NOT NULL DEFAULT 0,
        bagian_shahibul_maal DECIMAL(18,2) NOT NULL DEFAULT 0,
        status_distribusi VARCHAR(20) NOT NULL DEFAULT 'BELUM',
    ...
    // Create koperasi_shu
    await queryPostgres(`
      CREATE TABLE IF NOT EXISTS koperasi_shu (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        org_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
        tahun INT NOT NULL,
        total_shu DECIMAL(18,2) NOT NULL DEFAULT 0,
        cadangan DECIMAL(18,2) NOT NULL DEFAULT 0,
        anggota DECIMAL(18,2) NOT NULL DEFAULT 0,
        pengurus DECIMAL(18,2) NOT NULL DEFAULT 0,
        dana_sosial DECIMAL(18,2) NOT NULL DEFAULT 0,
        pendidikan DECIMAL(18,2) NOT NULL DEFAULT 0,
        tgl_hitung DATE NOT NULL DEFAULT CURRENT_DATE,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE(org_id, tahun)
      )
    ...
    // RPC function for generating kode anggota
    await queryPostgres(`
      ...
      BEGIN
        ...
        v_kode := 'KOP-' || TO_CHAR(NOW(), 'YYMM') || '-' || LPAD(v_count::TEXT, 4, '0');
        RETURN v_kode;
      END;
      $$ LANGUAGE plpgsql
    `)

    return NextResponse.json({ message: 'Migration completed ✓ All koperasi tables created' })
  } catch (err: any) {
