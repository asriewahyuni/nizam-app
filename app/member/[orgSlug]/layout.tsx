...
export default async function MemberPortalLayout({ children, params }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[#f7fbf9] text-slate-950 antialiased">
      <a
        href="#konten-utama"
      ...
      <header className="sticky top-0 z-50 border-b border-emerald-950/10 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/85">
        <div className="mx-auto flex min-h-16 max-w-7xl items-center gap-4 px-4 sm:px-6 lg:px-8">
          <Link
            href={basePath || '/'}
            className="group flex min-h-11 min-w-0 cursor-pointer items-center gap-3 rounded-lg focus:outline-none focus-visible:ring-4 focus-visible:ring-emerald-200"
          >
            <span className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-emerald-700 text-white shadow-sm transition-colors duration-200 group-hover:bg-emerald-800">
              <BookOpen aria-hidden="true" size={21} />
            </span>
            <span className="min-w-0">
              <span className="block truncate font-semibold tracking-tight">{tenant.portalName}</span>
              <span className="block text-xs font-medium text-emerald-800">Portal member resmi</span>
            </span>
          </Link>

          <nav
            aria-label="Navigasi utama"
            className="ml-auto hidden items-center gap-1 2xl:flex"
          >
            {navigation.map((item) => {
              ...
              return (
                <Link
                  key={item.label}
                  href={item.href}
                  className="flex min-h-11 cursor-pointer items-center gap-2 rounded-lg px-3 text-sm font-semibold text-slate-600 transition-colors duration-200 hover:bg-emerald-50 hover:text-emerald-900 focus:outline-none focus-visible:ring-4 focus-visible:ring-emerald-200"
                >
          ...
          {member ? (
            ...
            >
              ...
              <span className="hidden max-w-32 truncate sm:inline">{member.displayName}</span>
            </Link>
          ) : (
            <Link
        ...
        <nav
          ...
          className="flex gap-1 overflow-x-auto border-t border-slate-100 px-3 py-2 2xl:hidden"
        >
          {navigation.map((item) => {
            const Icon = item.icon
            return (
              <Link
                key={item.label}
                href={item.href}
                className="flex min-h-11 shrink-0 cursor-pointer items-center gap-2 rounded-lg px-3 text-sm font-semibold text-slate-600 transition-colors duration-200 hover:bg-emerald-50 hover:text-emerald-900 focus:outline-none focus-visible:ring-4 focus-visible:ring-emerald-200"
              >
                <Icon aria-hidden="true" size={17} />
                {item.label}
              </Link>
      ...
      <main id="konten-utama" className="min-h-[calc(100vh-12rem)]">
        {children}
      </main>
      ...
      <footer className="border-t border-emerald-950/10 bg-emerald-950 text-emerald-50">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-8 sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8">
          <div>
            ...
            <p className="mt-1 text-sm text-emerald-100/75">Pembelajaran dan transaksi dilindungi Nizam ERP.</p>
          </div>
