-- ============================================================
-- MIGRATION 1220: Harden Sales Delivery Journal Idempotency
-- ============================================================
-- Why:
-- Sebagian data legacy dapat menyisakan jurnal delivery `SALE` walaupun
-- status sales belum sinkron ke `FINISHED`. Saat delivery dicoba ulang,
-- constraint `uq_journal_ref_per_org` melempar duplicate key mentah.
--
-- Fix:
-- 1) Deteksi lebih awal bila jurnal `SALE` untuk sales ini sudah ada.
-- 2) Auto-heal status sales menjadi `FINISHED` bila jurnal dan stock
--    movement memang sudah ada.
-- 3) Keluarkan error bisnis yang jelas bila jurnal lama ada tetapi
--    sinkronisasi stok belum lengkap.
...
DECLARE
    ...
    v_existing_entry_status journal_status;
    v_has_existing_sale_movements BOOLEAN := FALSE;
BEGIN
    ...
    INTO v_existing_entry_id, v_existing_entry_status
    ...
    IF v_requires_inventory_sync THEN
      ...
      INTO v_has_existing_sale_movements;
    END IF;

    IF v_existing_entry_id IS NOT NULL THEN
      IF v_existing_entry_status = 'VOIDED' THEN
        RAISE EXCEPTION 'Sales order % sudah memiliki jurnal delivery yang sudah di-void. Rekonsiliasi jurnal lama sebelum mengirim ulang.', v_sale.sale_number
          USING ERRCODE = 'P0001';
      END IF;

      IF (NOT v_requires_inventory_sync) OR v_has_existing_sale_movements THEN
        UPDATE public.sales
        SET status = 'FINISHED',
      ...
      RAISE EXCEPTION 'Sales order % sudah memiliki jurnal delivery, tetapi mutasi stok belum ditemukan. Rekonsiliasi data lama sebelum mengirim ulang.', v_sale.sale_number
        USING ERRCODE = 'P0001';
    END IF;
