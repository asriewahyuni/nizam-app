-- ============================================================
-- MIGRATION 1213: Refresh Shariah CoA Injection for SALAM/ISTISHNA
-- ============================================================
-- Goals:
-- 1) Re-enable CoAS injection so existing inactive accounts become active again
-- 2) Ensure SALAM + ISTISHNA operational accounts are seeded together
-- 3) Keep legacy 3100 disabled while preserving Syirkah under 3000

DROP FUNCTION IF EXISTS public.inject_shariah_coa(UUID);

CREATE FUNCTION public.inject_shariah_coa(p_org_id UUID)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Cleanup legacy equity parent 3100 only.
  ...
  -- Liability parent and operational SALAM/ISTISHNA liabilities.
  INSERT INTO public.accounts (org_id, code, name, type, normal_balance, parent_id, is_system, is_active)
  ...
  -- Current-asset side for SALAM/ISTISHNA purchasing.
  INSERT INTO public.accounts (org_id, code, name, type, normal_balance, parent_id, is_system, is_active)
  VALUES
  ...
  FOR v_org IN
    (
      ...
      FROM (
        ...
        WHERE a.code IN ('3100', '3110', '3120', '2600', '2601', '2602', '2603', '1404', '1205', '6100', '6200')
        UNION
        SELECT s.org_id
        FROM public.sales s
