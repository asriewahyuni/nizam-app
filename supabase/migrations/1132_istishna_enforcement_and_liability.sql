-- ============================================================
-- MIGRATION 1132: ISTISHNA Enforcement + Liability Account
-- ============================================================
-- Goals:
-- 1) Provide 'Hutang Istishna' (2603) account for Istishna contracts
-- 2) Update payment atomic function to route Istishna payments to Hutang Istishna
-- 3) Update delivery atomic function to debit Hutang Istishna and balance to AR
-- ============================================================

CREATE OR REPLACE FUNCTION public.ensure_istishna_liability_account(p_org_id UUID)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Seed existing organizations with Hutang Istishna
DO $$
DECLARE
  v_org_id UUID;
BEGIN
  FOR v_org_id IN
    (
      ...
      FROM public.sales
      WHERE UPPER(COALESCE(shariah_mode::TEXT, 'CASH')) = 'ISTISHNA'
      UNION
      SELECT DISTINCT org_id
      FROM public.accounts
      WHERE code = '2600'
  ...
  LOOP
    PERFORM public.ensure_istishna_liability_account(v_org_id);
  END LOOP;
END;
$$;
...
BEGIN
    ...
    ELSIF v_is_istishna THEN
      IF coalesce(v_sale.status, '') = 'FINISHED' THEN
...
GRANT EXECUTE ON FUNCTION public.ensure_istishna_liability_account(UUID) TO authenticated;
