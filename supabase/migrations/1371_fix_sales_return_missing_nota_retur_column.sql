-- process_sales_return_atomic() masih meng-INSERT ke kolom sales_returns.nota_retur_number,
-- tapi kolom itu sudah tidak ada di tabel sales_returns yang sebenarnya (skema live) —
-- menyebabkan setiap retur penjualan gagal dengan error
-- "column "nota_retur_number" of relation "sales_returns" does not exist".
--
-- Migration 023_fix_inventory_average_cost.sql sudah pernah memperbaiki masalah yang sama
-- dengan menyimpan nomor nota retur di kolom `notes` — perbaikan ini mengembalikan pola itu
-- (migration 1091/1108/1120 tidak sengaja meregresi balik ke kolom yang sudah tidak ada).
