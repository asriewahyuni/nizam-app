...
- Nama dokter yang tampil di bawah nama poli diambil dari `klinik_jadwal_praktik` yang **aktif saat itu juga** (weekday + jam berjalan, dikurangi `klinik_jadwal_pengecualian` berstatus `BLOCKED`) — bukan dari `klinik_kunjungan.staf_medis_id` (kosong untuk walk-in) atau `klinik_rekam_medis.staf_medis_id` (baru terisi setelah pemeriksaan dimulai). Ini supaya nama dokter selalu tampil begitu jam praktik poli itu buka, tidak menunggu ada pasien yang sedang diperiksa dulu.
- Tombol di kartu "Antrian Hari Ini" (tab Pendaftaran & Antrian): ikon buka-tab-baru (`ExternalLink`) dan ikon salin-link (`Copy`, dengan umpan balik instan berubah jadi centang saat berhasil — pola micro-delight yang sama dipakai di seluruh app untuk elemen "disalin").
