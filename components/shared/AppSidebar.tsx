...
const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Whatslab CRM', href: '/wacrm', icon: MessageCircle, permission_key: 'crm', module_key: 'WA_CRM' },
  ...
  return (
    <>
      ...
      <div className={`p-3 border-t border-slate-100 bg-white ${effectiveIsCollapsed ? 'flex flex-col items-center gap-2' : 'flex items-center justify-between w-full gap-3'}`}>
        <div ref={accountMenuRef} className={`relative ${effectiveIsCollapsed ? '' : 'min-w-0 flex-1'}`}>
          <button
            type="button"
            onClick={() => setShowAccountMenu(v => !v)}
            title="Akun saya"
            className={`flex items-center gap-3 w-full rounded-md p-1 -m-1 text-left transition-colors duration-150 cursor-pointer hover:bg-slate-100 ${effectiveIsCollapsed ? 'justify-center' : ''}`}
          >
            <div className="w-8 h-8 shrink-0 rounded-full bg-slate-700 overflow-hidden flex items-center justify-center text-xs font-semibold text-white relative">
          ...
          {showAccountMenu && (
            <div className="absolute bottom-full left-0 mb-2 w-52 bg-white rounded-lg border border-slate-200 shadow-lg shadow-slate-900/10 overflow-hidden z-50">
              <Link
                href="/profil-saya"
                onClick={() => setShowAccountMenu(false)}
                className="flex items-center gap-2.5 px-3.5 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors duration-150 cursor-pointer"
              >
                <User size={15} className="text-slate-400" /> Profil Saya
              </Link>
              <button
                type="button"
                onClick={() => { setShowAccountMenu(false); setShowChangePasswordModal(true) }}
                className="flex items-center gap-2.5 w-full px-3.5 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors duration-150 cursor-pointer text-left"
              >
                <KeyRound size={15} className="text-slate-400" /> Ganti Password
              </button>
            </div>
          )}
        </div>
        ...
        <button
          ...
          className={`shrink-0 p-2 rounded-md transition-colors duration-150 cursor-pointer disabled:cursor-wait disabled:opacity-60 ${isDemo ? 'text-orange-500 hover:text-orange-600 hover:bg-orange-50' : 'text-slate-400 hover:text-rose-600 hover:bg-rose-50'}`}
        >
