...
export function SafeButton({
  ...
  const variants = {
    primary: 'bg-blue-600 hover:bg-blue-700 text-white shadow-sm border border-blue-700/10',
    danger: 'bg-rose-600 hover:bg-rose-700 text-white shadow-sm border border-rose-700/10',
    ...
    indigo: 'bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm border border-indigo-700/10',
    emerald: 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm border border-emerald-700/10',
    amber: 'bg-amber-600 hover:bg-amber-700 text-white shadow-sm border border-amber-700/10',
    blue: 'bg-sky-600 hover:bg-sky-700 text-white shadow-sm border border-sky-700/10',
    secondary: 'bg-slate-800 hover:bg-slate-900 text-white shadow-sm border border-slate-700'
  }
  ...
  const sizes = {
    ...
    xl: 'px-6 py-3 text-base gap-2.5 rounded-lg'
  }
  ...
  return (
    <button
      ...
      className={`
        inline-flex items-center justify-center gap-2 font-semibold transition-colors duration-150 cursor-pointer active:scale-[0.98]
    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-200">
      <div className="space-y-1">
        <div className="flex items-center gap-3">
          {icon && (
            <div className={`p-2 rounded-lg bg-white shadow-sm border border-slate-200 ${iconColor}`}>
              {typeof icon === 'function' ?
                React.createElement(icon as React.ComponentType<any>, { size: 20, strokeWidth: 2 }) :
          ...
          <div className="space-y-0.5">
            {tag && (
              <div className="flex items-center gap-1.5 text-indigo-600 font-medium text-[10px] uppercase bg-indigo-50 w-fit px-2 py-0.5 rounded border border-indigo-100 mb-0.5 tracking-wide">
                {tag}
              </div>
            )}
        ...
        {subtitle && <p className="text-sm text-slate-500 font-medium pl-1">{subtitle}</p>}
      </div>
...
export function StatCard({ label, value, sub, color = 'slate', icon: Icon, alert, trend, onClick, href }: StatCardProps) {
  ...
  const card = (
    <div
      onClick={onClick}
      className={`p-4 rounded-xl border shadow-sm transition-colors duration-150 relative overflow-hidden flex flex-col justify-between h-full min-h-[100px] ${styles[color]} ${isInteractive ? 'cursor-pointer hover:shadow-md group/card' : 'group'}`}
    >
      <div className="flex items-start justify-between mb-2 relative z-10">
        <div className="space-y-1 flex-1 min-w-0 pr-2">
          <p className={`text-[10px] font-semibold uppercase tracking-wide ${isOrange ? 'text-white/80' : 'text-slate-500'}`}>
            {label}
          </p>
          <div className={`text-lg font-bold font-mono tracking-tight truncate ${isOrange ? 'text-white' : 'text-slate-900'}`}>{value}</div>
        </div>
        {Icon && (
...
export function EmptyState({ icon: Icon, title, description, action }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-12 gap-4 text-center bg-slate-50/50 rounded-xl border border-dashed border-slate-200">
      {Icon && (
        <div className="w-12 h-12 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-300 shadow-sm">
...
export function SectionCard({ children, className = '', glass = false }: { children: React.ReactNode; className?: string; glass?: boolean }) {
  return (
    <div className={`
      ...
      ${glass ? 'bg-white/80 backdrop-blur-xl border-white/50' : 'bg-white border-slate-200'}
      ${className}
    `}>
      {children}
    </div>
...
export function ConfirmDialog({ isOpen, onClose, onConfirm, title, message, confirmLabel = 'Konfirmasi', variant = 'danger' }: ConfirmDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm" onClick={onClose} />
      <motion.div initial={{ opacity: 0, scale: 0.97, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.15, ease: 'easeOut' }} className="relative bg-white rounded-xl shadow-lg border border-slate-200 p-6 w-full max-w-sm space-y-5 text-center overflow-hidden">
