    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        <SectionCard>
          <div className="p-5 space-y-2">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">User Aktif 10 Menit</p>
            <p className="text-3xl font-black tracking-tight text-slate-900">{snapshot?.summary.activeUsers10m ?? 0}</p>
          </div>
        </SectionCard>
        ...
        <SectionCard>
          <div className="p-5 space-y-2">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Kunjungan Route 24 Jam</p>
            <p className="text-3xl font-black tracking-tight text-slate-900">{snapshot?.summary.routeVisits24h ?? 0}</p>
          </div>
        </SectionCard>
        ...
        <SectionCard>
          <div className="p-5 space-y-2">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">User Login 24 Jam</p>
            <p className="text-3xl font-black tracking-tight text-slate-900">{snapshot?.summary.usersLoggedIn24h ?? 0}</p>
          </div>
        </SectionCard>
        ...
        <SectionCard>
          <div className="p-5 space-y-2">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">User Unik Route 24 Jam</p>
            <p className="text-3xl font-black tracking-tight text-slate-900">{snapshot?.summary.uniqueUsers24h ?? 0}</p>
          </div>
        </SectionCard>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.3fr,0.7fr] gap-6">
        <SectionCard>
          <div className="p-6 space-y-5">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                <h3 className="text-xl font-black tracking-tight text-slate-900">User Aktif Sekarang</h3>
                <p className="mt-1 text-sm font-medium text-slate-500">
                  Dianggap aktif bila ada activity route atau heartbeat dalam 10 menit terakhir.
                </p>
              </div>
              <div className="flex items-center gap-3">
                <div className="text-right text-[11px] font-black uppercase tracking-[0.14em] text-slate-400">
                  Update terakhir
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredCurrentUsers.map((user) => (
                  <div key={`${user.actorUserId || user.email || 'unknown'}:${user.occurredAt}`} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <div className="mt-4 space-y-3 text-sm">
                      <div className="rounded-2xl bg-slate-50 px-4 py-3">
                        <div className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-400">Route Terakhir</div>
                        <div className="mt-1 font-black text-slate-900 break-all">{user.lastRouteFull || user.lastRoutePath || '-'}</div>
                      </div>
        ...
        <SectionCard>
          <div className="p-6 space-y-5">
            <div>
              <h3 className="text-xl font-black tracking-tight text-slate-900">Route Teratas 24 Jam</h3>
              <p className="mt-1 text-sm font-medium text-slate-500">
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {(snapshot?.topRoutes || []).map((route, index) => (
                  <div key={`${route.routePath}:${index}`} className="rounded-2xl border border-slate-200 bg-slate-50/70 px-4 py-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">#{index + 1}</div>
                        <div className="mt-1 text-sm font-black text-slate-900 break-all">{route.routePath}</div>
                      </div>
                      <div className="text-right shrink-0">
                        <div className="text-lg font-black text-slate-900">{route.visits}</div>
      ...
      <SectionCard>
        <div className="p-6 space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h3 className="text-xl font-black tracking-tight text-slate-900">Log Route Terbaru</h3>
              <p className="mt-1 text-sm font-medium text-slate-500">
            ...
            <div className="relative w-full lg:max-w-md">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-slate-200 bg-white py-3.5 pl-12 pr-4 font-bold text-slate-700 shadow-sm outline-none transition focus:border-blue-300 focus:ring-2 focus:ring-blue-100"
              />
            </div>
          </div>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-slate-100">
                    <th className="px-4 py-4 text-left text-[11px] font-black uppercase tracking-widest text-slate-400">Waktu</th>
                    <th className="px-4 py-4 text-left text-[11px] font-black uppercase tracking-widest text-slate-400">User</th>
                    <th className="px-4 py-4 text-left text-[11px] font-black uppercase tracking-widest text-slate-400">Tenant / Unit</th>
                    <th className="px-4 py-4 text-left text-[11px] font-black uppercase tracking-widest text-slate-400">Route</th>
                    <th className="px-4 py-4 text-left text-[11px] font-black uppercase tracking-widest text-slate-400">Akses</th>
                  </tr>
                </thead>
                  ...
                  {filteredActivities.map((activity) => (
                    <tr key={activity.id} className="hover:bg-slate-50/70 transition-colors">
                      <td className="px-4 py-4 align-top">
                        <div className="text-sm font-black text-slate-900">{formatDateTime(activity.occurredAt)}</div>
                        <div className="mt-1 flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.14em] text-slate-400">
                          <Clock3 size={11} />
                          {formatRelativeAge(activity.occurredAt)}
                        </div>
                      </td>
                      <td className="px-4 py-4 align-top">
                        <div className="text-sm font-black text-slate-900">{activity.displayName || activity.email || 'User tanpa nama'}</div>
                      ...
                      <td className="px-4 py-4 align-top">
                        <div className="rounded-2xl bg-slate-50 px-3 py-2 text-sm font-black text-slate-900 break-all">
