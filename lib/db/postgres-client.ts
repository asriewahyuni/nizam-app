...
class PostgresQueryBuilder {
  ...
  private async _resolveNestedRelations(
    ...
    for (const rel of relations) {
      ...
      const aliasMatch = rel.match(/^(\w+):(\w+)\((.*)\)$/s)
      const simpleMatch = rel.match(/^(\w+)\((.*)\)$/s)
      if (!aliasMatch && !simpleMatch) continue

      const alias = aliasMatch ? aliasMatch[1] : simpleMatch![1]
      const relTable = aliasMatch ? aliasMatch[2] : simpleMatch![1]
      const relCols = aliasMatch ? aliasMatch[3] : simpleMatch![2]
      const relColExpr = relCols.trim() === '*' ? '*' : relCols
      ...
      try {
        ...
        if (fkCol) {
          ...
          if (foreignIds.length > 0) {
            const relResult = await queryPostgres<Record<string, unknown>>(
              'SELECT ' + relColExpr + ' FROM ' + relTableQ + ' WHERE id = ANY($1::uuid[])',
              [foreignIds]
            )
        ...
        if (parentIds.length > 0) {
          try {
            const relResult = await queryPostgres<Record<string, unknown>>(
              'SELECT ' + relColExpr + ' FROM ' + relTableQ + ' WHERE "' + backRefColFromCache + '" = ANY($1::uuid[])',
              [parentIds]
            )
...
async function callRpc(
  ...
  try {
    ...
    const paramValues = args.map(([, v]) => v)
    const paramNames = args.map(([k]) => `${k} => $${args.indexOf([k, args.find(([pk]) => pk === k)![1]] as [string, unknown]) + 1}`)
